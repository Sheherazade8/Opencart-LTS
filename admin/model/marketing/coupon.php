...
class ModelMarketingCoupon extends Model {
	public function addCoupon($data) {
		...
		if (isset($data['coupon_assessment'])) {
			foreach ($data['coupon_assessment'] as $assessment_id) {
				$this->db->query("INSERT INTO " . DB_PREFIX . "coupon_assessment SET coupon_id = '" . (int)$coupon_id . "', assessment_id = '" . (int)$assessment_id . "'");
			}
		}

		if (isset($data['coupon_exam'])) {
			foreach ($data['coupon_exam'] as $exam_id) {
				$this->db->query("INSERT INTO " . DB_PREFIX . "coupon_exam SET coupon_id = '" . (int)$coupon_id . "', exam_id = '" . (int)$exam_id . "'");
			}
		}
	...
	public function editCoupon($coupon_id, $data) {
		...
		$this->db->query("DELETE FROM " . DB_PREFIX . "coupon_assessment WHERE coupon_id = '" . (int)$coupon_id . "'");
		...
		$this->db->query("DELETE FROM " . DB_PREFIX . "coupon_exam WHERE coupon_id = '" . (int)$coupon_id . "'");
	...
	public function getCouponAssessments($coupon_id) {
		$coupon_assessment_data = array();

		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "coupon_assessment WHERE coupon_id = '" . (int)$coupon_id . "'");

		foreach ($query->rows as $result) {
			$coupon_assessment_data[] = $result['assessment_id'];
		}

		return $coupon_assessment_data;
	}

	public function getCouponExams($coupon_id) {
		$coupon_exam_data = array();

		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "coupon_exam WHERE coupon_id = '" . (int)$coupon_id . "'");

		foreach ($query->rows as $result) {
			$coupon_exam_data[] = $result['exam_id'];
		}

		return $coupon_exam_data;
	}

...
class ModelExtensionFeedGoogleBase extends Model {
    ...
    public function getExams($data = array()) {
        $sql = "SELECT google_base_exam_id, (SELECT name FROM `" . DB_PREFIX . "google_base_exam` gbc WHERE gbc.google_base_exam_id = gbc2c.google_base_exam_id) AS google_base_exam, exam_id, (SELECT name FROM `" . DB_PREFIX . "exam` e WHERE e.exam_id = gbc2c.exam_id) AS exam FROM `" . DB_PREFIX . "google_base_exam_to_exam` gbc2c ORDER BY google_base_exam ASC";

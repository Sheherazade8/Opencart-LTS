...
class ControllerMarketingCoupon extends Controller {
	...
	protected function getForm() {
		...
		if (isset($this->request->post['coupon_assessment'])) {
			$assessments = $this->request->post['coupon_assessment'];
		} elseif (isset($this->request->get['coupon_id'])) {
			$assessments = $this->model_marketing_coupon->getCouponAssessments($this->request->get['coupon_id']);
		} else {
			$assessments = array();
		}
		...
		if (isset($this->request->post['coupon_exam'])) {
			$exams = $this->request->post['coupon_exam'];
		} elseif (isset($this->request->get['coupon_id'])) {
			$exams = $this->model_marketing_coupon->getCouponExams($this->request->get['coupon_id']);
		} else {
			$exams = array();
		}
		...
		foreach ($exams as $exam_id) {
			...
			if ($exam_info) {
				$data['coupon_exam'][] = array(
					...
					'name'        => ($exam_info['path'] ? $exam_info['path'] . ' &gt; ' : '') . $exam_info['name']
				);
			}
		}

...
class ControllerSaleOrder extends Controller {
	...
	public function getForm() {
		...
		if (!empty($order_info)) {
			...
			// Assessments
			$data['order_assessments'] = array();

			$assessments = $this->model_sale_order->getOrderAssessments($this->request->get['order_id']);
			...
			foreach ($assessments as $assessment) {
				$data['order_assessments'][] = array(
					...
					'option'     => $this->model_sale_order->getOrderOptions($this->request->get['order_id'], $assessment['order_assessment_id']),
	...
	public function info() {
		...
		if ($order_info) {
			...
			foreach ($assessments as $assessment) {
				...
				$options = $this->model_sale_order->getOrderOptions($this->request->get['order_id'], $assessment['order_assessment_id']);
				...
				$data['assessments'][] = array(
					'order_assessment_id' => $assessment['order_assessment_id'],
					'assessment_id'       => $assessment['assessment_id'],
					'name'    	 	   => $assessment['name'],
					'model'    		   => $assessment['model'],
					'option'   		   => $option_data,
					...
					'href'     		   => $this->url->link('catalog/assessment/edit', 'user_token=' . $this->session->data['user_token'] . '&assessment_id=' . $assessment['assessment_id'], true)
				);
			}
	...
	public function invoice() {
		...
		foreach ($orders as $order_id) {
			...
			if ($order_info) {
				...
				$assessments = $this->model_sale_order->getOrderAssessments($order_id);

				foreach ($assessments as $assessment) {
					$option_data = array();

					$options = $this->model_sale_order->getOrderOptions($order_id, $assessment['order_assessment_id']);
				...
				foreach ($assessments as $assessment) {
					...
					if ($assessment_info) {
						...
						foreach ($options as $option) {
							...
							$assessment_option_value_info = $this->model_catalog_assessment->getAssessmentOptionValue($assessment['assessment_id'], $option['assessment_option_value_id']);

							if (!empty($assessment_option_value_info['weight'])) {
								if ($assessment_option_value_info['weight_prefix'] == '+') {
									$option_weight += $assessment_option_value_info['weight'];
								} elseif ($assessment_option_value_info['weight_prefix'] == '-') {
									$option_weight -= $assessment_option_value_info['weight'];
								}
							}
						}
						...
						$assessment_data[] = array(
							...
							'option'   => $option_data,
							'quantity' => $assessment['quantity'],
							'location' => $assessment_info['location'],
						// 	'sku'      => $assessment_info['sku'],
						// 	'upc'      => $assessment_info['upc'],
						// 	'ean'      => $assessment_info['ean'],
						// 	'jan'      => $assessment_info['jan'],
						// 	'isbn'     => $assessment_info['isbn'],
						// 	'mpn'      => $assessment_info['mpn'],
						// 	'weight'   => $this->weight->format(($assessment_info['weight'] + (float)$option_weight) * $assessment['quantity'], $assessment_info['weight_class_id'], $this->language->get('decimal_point'), $this->language->get('thousand_point'))
						);
					}
				}

...
class ControllerExtensionFeedGoogleBase extends Controller {
	...
	public function exam() {
		...
		$data['google_base_exams'] = array();

		$this->load->model('extension/feed/google_base');

		$results = $this->model_extension_feed_google_base->getExams(($page - 1) * 10, 10);

		foreach ($results as $result) {
			$data['google_base_exams'][] = array(
				'google_base_exam_id' => $result['google_base_exam_id'],
				'google_base_exam'    => $result['google_base_exam'],
				'exam_id'             => $result['exam_id'],
				'exam'                => $result['exam']
			);
		}

		$exam_total = $this->model_extension_feed_google_base->getTotalExams();

		$pagination = new Pagination();
		$pagination->total = $exam_total;
		$pagination->page = $page;
		$pagination->limit = 10;
		$pagination->url = $this->url->link('extension/feed/google_base/exam', 'user_token=' . $this->session->data['user_token'] . '&page={page}', true);

		$data['pagination'] = $pagination->render();

		$data['results'] = sprintf($this->language->get('text_pagination'), ($exam_total) ? (($page - 1) * 10) + 1 : 0, ((($page - 1) * 10) > ($exam_total - 10)) ? $exam_total : ((($page - 1) * 10) + 10), $exam_total, ceil($exam_total / 10));

		$this->response->setOutput($this->load->view('extension/feed/google_base_exam', $data));
	}
	...
	public function addExam() {
		...
		if (!$this->user->hasPermission('modify', 'extension/feed/google_base')) {
			$json['error'] = $this->language->get('error_permission');
		} elseif (!empty($this->request->post['google_base_exam_id']) && !empty($this->request->post['exam_id'])) {
			$this->load->model('extension/feed/google_base');

			$this->model_extension_feed_google_base->addExam($this->request->post);
	...
	public function removeExam() {
		...
		if (!$this->user->hasPermission('modify', 'extension/feed/google_base')) {
			...
			$this->model_extension_feed_google_base->deleteExam($this->request->post['exam_id']);
	...
	public function autocomplete() {
		...
		if (isset($this->request->get['filter_name'])) {
			...
			$results = $this->model_extension_feed_google_base->getGoogleBaseExams($filter_data);

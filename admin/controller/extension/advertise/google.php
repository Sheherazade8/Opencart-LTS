...
class ControllerExtensionAdvertiseGoogle extends Controller {
    ...
    public function index() {
        ...
        if (!$this->setting->get('advertise_google_status') && $this->model_extension_advertise_google->hasActiveTarget($this->store_id)) {
            ...
            $data['warning'] = sprintf($this->language->get("warning_no_advertised_assessments"), $this->language->get("text_video_tutorial_url_advertise"));
        ...
        $data['url_popup'] = html_entity_decode($this->url->link('extension/advertise/google/popup_assessment', 'store_id=' . $this->store_id . '&user_token=' . $this->session->data['user_token'], true), ENT_QUOTES, 'UTF-8');
        $data['url_exam_autocomplete'] = html_entity_decode($this->url->link('extension/advertise/google/exam_autocomplete', 'store_id=' . $this->store_id . '&user_token=' . $this->session->data['user_token'], true), ENT_QUOTES, 'UTF-8');
    ...
    public function advertise() {
        ...
        if (!$this->setting->get('advertise_google_status') && $this->model_extension_advertise_google->hasActiveTarget($this->store_id)) {
            ...
            $json['warning'] = sprintf($this->language->get("warning_no_advertised_assessments"), $this->language->get("text_video_tutorial_url_advertise"));
        } else if ($last_cron_executed + 24 * 60 * 60 <= time()) {
    ...
    public function list_ads() {
        ...
        $this->model_extension_advertise_google->insertNewAssessments(array(), $this->store_id);
        ...
        $assessments = $this->googleshopping->getAssessments($filter_data, $this->store_id);

        $json['assessments'] = array_map(array($this, 'assessment'), $assessments);

        $assessment_total = $this->googleshopping->getTotalAssessments($filter_data, $this->store_id);
        ...
        $pages = ceil($assessment_total / $this->config->get('config_limit_admin'));

        $json['showing'] = sprintf($this->language->get('text_pagination'), ($assessment_total) ? (($page - 1) * $this->config->get('config_limit_admin')) + 1 : 0, ((($page - 1) * $this->config->get('config_limit_admin')) > ($assessment_total - $this->config->get('config_limit_admin'))) ? $assessment_total : ((($page - 1) * $this->config->get('config_limit_admin')) + $this->config->get('config_limit_admin')), $assessment_total, $pages);

        $json['pagination'] = $pagination->render();
        $json['total'] = (int)$assessment_total;
    ...
    public function mapping() {
        ...
        if ($this->request->server['REQUEST_METHOD'] == 'POST' && $this->validateMapping()) {
            try {
                foreach ($this->request->post['advertise_google_mapping'] as $google_assessment_exam => $exam_id) {
                    $this->model_extension_advertise_google->setExamMapping($google_assessment_exam, $this->store_id, $exam_id);
                }

                if (!empty($this->request->post['advertise_google_modify_existing'])) {
                    $this->model_extension_advertise_google->updateGoogleAssessmentExamMapping($this->store_id);
                }
        ...
        foreach ($this->config->get('advertise_google_google_assessment_exams') as $google_assessment_exam_id => $google_assessment_exam_name) {
            if ($google_assessment_exam_id == 0) continue;

            $exam_id = '';
            $name = '';

            if (null !== $exam = $this->model_extension_advertise_google->getMappedExam($google_assessment_exam_id, $this->store_id)) {
                $exam_id = $exam['exam_id'];
                $name = $exam['name'];
            }

            $map = array(
                'google_assessment_exam' => array(
                    'id' => $google_assessment_exam_id,
                    'name' => $google_assessment_exam_name
                ),
                'oc_exam' => array(
                    'exam_id' => $exam_id,
                    'name' => $name
    ...
    public function mapping_verify() {
        ...
        $json = array(
            'submit_directly' => !$this->model_extension_advertise_google->isAnyAssessmentExamModified($this->store_id),
    ...
    public function popup_assessment() {
        $json = array(
            'body' => '',
            'title' => '',
            'success' => false,
        ...
        if ($this->request->post['operand']['type'] == 'single') {
            $assessment_advertise_google_id = $this->request->post['operand']['data'];

            $assessment_info = $this->model_extension_advertise_google->getAssessmentByAssessmentAdvertiseGoogleId($assessment_advertise_google_id);

            if ($assessment_info !== NULL) {
                $json['assessment_id'] = $assessment_info['assessment_id'];
                
                // Required variables:
                $operand_info = array(
                    'title' => sprintf($this->language->get('text_popup_title_single'), $assessment_info['name'], $assessment_info['model'])
                );

                $required_fields = $this->model_extension_advertise_google->getRequiredFieldsByAssessmentIds(array($assessment_info['assessment_id']), $this->store_id);
                ...
                $options = $this->model_extension_advertise_google->getAssessmentOptionsByAssessmentIds(array($assessment_info['assessment_id']));

                $default_form_data = $this->model_extension_advertise_google->getAssessmentAdvertiseGoogle($assessment_advertise_google_id);
            }
            ...
            if (!empty($this->request->post['operand']['data']['all_pages'])) {
                ...
                $total_assessments = $this->googleshopping->getTotalAssessments($filter_data, $this->store_id);

                // Required variables:
                $operand_info = array(
                    'title' => sprintf($this->language->get('text_popup_title_multiple'), $total_assessments)
                );
                ...
                $options = $this->model_extension_advertise_google->getAssessmentOptionsByFilter($filter_data);
            } else {
                $assessment_ids = $this->request->post['operand']['data']['select'];

                $total_assessments = count($assessment_ids);
                ...
                $required_fields = $this->model_extension_advertise_google->getRequiredFieldsByAssessmentIds($assessment_ids, $this->store_id);

                if ($this->request->post['action'] == 'submit') {
                    $form_data = $this->request->post['form'];
                }

                $options = $this->model_extension_advertise_google->getAssessmentOptionsByAssessmentIds($assessment_ids);
            }
        ...
        if ($operand_info !== NULL) {
            ...
            $json['success_message'] = $this->language->get('success_assessment');

            $this->load->config('googleshopping/googleshopping');

            $json['required_fields'] = $required_fields;

            if ($this->request->post['action'] == 'submit' && $this->validateAssessment($required_fields)) {
                $form_data['store_id'] = (int)$this->store_id;

                if ($this->request->post['operand']['type'] == 'single') {
                    $this->model_extension_advertise_google->updateSingleAssessmentFields($form_data);
                } else if ($this->request->post['operand']['type'] == 'multiple') {
                    if (!empty($this->request->post['operand']['data']['all_pages'])) {
                        $this->model_extension_advertise_google->updateMultipleAssessmentFields($filter_data, $form_data);
                    } else {
                        foreach ($assessment_ids as $assessment_id) {
                            $form_data['assessment_id'] = (int)$assessment_id;
                            $this->model_extension_advertise_google->updateSingleAssessmentFields($form_data);
                        }
                    }
                }
            ...
            if (isset($this->error['assessment_exam'])) {
                $data['error_assessment_exam'] = $this->error['assessment_exam'];
            } else {
                $data['error_assessment_exam'] = '';
            }
            ...
            $data['google_assessment_exams'] = $this->config->get('advertise_google_google_assessment_exams');
            ...
            $data['google_assessment_exam'] = $form_data['google_assessment_exam'];
            ...
            $json['body'] = $this->load->view('extension/advertise/google_popup_assessment', $data);
        } else {
    ...
    public function popup_issues() {
        ...
        $assessment_id = isset($this->request->get['assessment_id']) ? (int)$this->request->get['assessment_id'] : 0;

        $assessment_issues = $this->model_extension_advertise_google->getAssessmentIssues($assessment_id, $this->store_id);

        if ($assessment_issues !== NULL) {
            $json['title'] = sprintf($this->language->get('text_popup_title_single'), $assessment_issues['name'], $assessment_issues['model']);

            $data['assessment_issues'] = $assessment_issues['entries'];
    ...
    public function addAssessment(&$route, &$args, &$output) {
        $this->load->model('extension/advertise/google');
        $this->load->model('catalog/assessment');

        foreach ($this->model_catalog_assessment->getAssessmentStores($output) as $store_id) {
            $this->model_extension_advertise_google->insertNewAssessments(array($output), $store_id);
        }
    }

    public function copyAssessment(&$route, &$args, &$output) {
        $this->load->model('extension/advertise/google');
        $this->load->model('catalog/assessment');

        $final_assessment_id = $this->model_extension_advertise_google->getFinalAssessmentId();

        if (!empty($final_assessment_id)) {
            foreach ($this->model_catalog_assessment->getAssessmentStores($final_assessment_id) as $store_id) {
                $this->model_extension_advertise_google->insertNewAssessments(array($final_assessment_id), $store_id);
            }
        }
    }

    public function deleteAssessment(&$route, &$args, &$output) {
        $this->load->model('extension/advertise/google');

        $this->model_extension_advertise_google->deleteAssessments(array((int)$args[0]));
    }
    ...
    public function exam_autocomplete() {
        $json = array();
        ...
        if (isset($this->request->get['filter_name'])) {
            ...
            $results = $this->model_extension_advertise_google->getExams($filter_data, $this->store_id);
    ...
    protected function getFilter($array) {
        if (!empty($array)) {
            return array(
                'filter_assessment_name' => $array['assessment_name'],
                'filter_assessment_model' => $array['assessment_model'],
                'filter_exam_id' => $array['exam_id'],
    ...
    protected function assessment(&$row) {        
        $this->load->config('googleshopping/googleshopping');
        ...
        return array(
            'assessment_advertise_google_id' => (int)$row['assessment_advertise_google_id'],
            ...
            'url_issues' => html_entity_decode($this->url->link('extension/advertise/google/popup_issues', 'store_id=' . $this->store_id . '&user_token=' . $this->session->data['user_token'] . '&assessment_id=' . $row['assessment_id'], true), ENT_QUOTES, 'UTF-8'),
            'campaigns' => $this->model_extension_advertise_google->getAssessmentCampaigns((int)$row['assessment_id'], $this->store_id)
        );
    }
    ...
    protected function validateAssessment($required_fields) {

...
class ModelCheckoutOrder extends Model {
	public function addOrder($data) {
		...
		if (isset($data['assessments'])) {
			foreach ($data['assessments'] as $assessment) {
				$this->db->query("INSERT INTO " . DB_PREFIX . "order_assessment SET order_id = '" . (int)$order_id . "', assessment_id = '" . (int)$assessment['assessment_id'] . "', name = '" . $this->db->escape($assessment['name']) . "', model = '" . $this->db->escape($assessment['model']) . "', quantity = '" . (int)$assessment['quantity'] . "', price = '" . (float)$assessment['price'] . "', total = '" . (float)$assessment['total'] . "', tax = '" . (float)$assessment['tax'] . "', reward = '" . (int)$assessment['reward'] . "'");

				$order_assessment_id = $this->db->getLastId();

				foreach ($assessment['option'] as $option) {
					$this->db->query("INSERT INTO " . DB_PREFIX . "order_option SET order_id = '" . (int)$order_id . "', order_assessment_id = '" . (int)$order_assessment_id . "', assessment_option_id = '" . (int)$option['assessment_option_id'] . "', assessment_option_value_id = '" . (int)$option['assessment_option_value_id'] . "', name = '" . $this->db->escape($option['name']) . "', `value` = '" . $this->db->escape($option['value']) . "', `type` = '" . $this->db->escape($option['type']) . "'");
				}
			}
		}
	...
	public function editOrder($order_id, $data) {
		...
		$this->db->query("DELETE FROM " . DB_PREFIX . "order_assessment WHERE order_id = '" . (int)$order_id . "'");
	...
	public function getOrderAssessments($order_id) {
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "order_assessment WHERE order_id = '" . (int)$order_id . "'");
		
		return $query->rows;
	}
	
	public function getOrderOptions($order_id, $order_assessment_id) {
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "order_option WHERE order_id = '" . (int)$order_id . "' AND order_assessment_id = '" . (int)$order_assessment_id . "'");
	...
	public function addOrderHistory($order_id, $order_status_id, $comment = '', $notify = false, $override = false) {
		...
		if ($order_info) {
			...
			if (!in_array($order_info['order_status_id'], array_merge($this->config->get('config_processing_status'), $this->config->get('config_complete_status'))) && in_array($order_status_id, array_merge($this->config->get('config_processing_status'), $this->config->get('config_complete_status')))) {
				...
				// Stock subtraction
				$order_assessments = $this->getOrderAssessments($order_id);

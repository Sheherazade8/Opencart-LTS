...
class ModelExtensionPaymentDivido extends Model {
	...
	public function getAssessmentSettings($assessment_id) {
		return $this->db->query("SELECT `display`, `plans` FROM `" . DB_PREFIX . "divido_assessment` WHERE `assessment_id` = '" . (int)$assessment_id . "'")->row;
	}
	...
	public function getCartPlans($cart)	{
		$plans = array();
		$assessments = $cart->getAssessments();
		foreach ($assessments as $assessment) {
			$assessment_plans = $this->getAssessmentPlans($assessment['assessment_id']);
			if ($assessment_plans) {
				$plans = array_merge($plans, $assessment_plans);
			}
		}
	...
	public function getAssessmentPlans($assessment_id) {
		$this->load->model('catalog/assessment');

		$assessment_info       = $this->model_catalog_assessment->getAssessment($assessment_id);
		$settings           = $this->getAssessmentSettings($assessment_id);
		$assessment_selection  = $this->config->get('payment_divido_assessmentselection');
		$divido_exams  = $this->config->get('payment_divido_exams');
		$price_threshold    = $this->config->get('payment_divido_price_threshold');

		if ($divido_exams) {
			$assessment_exams = $this->model_catalog_assessment->getExams($assessment_id);

			$all_exams = array();
			foreach ($assessment_exams as $assessment_exam) {
				$all_exams[] = $assessment_exam['exam_id'];
			}
			$exam_matches = array_intersect($all_exams, $divido_exams);
		...
		if ($assessment_selection == 'selected' && $settings['display'] == 'custom' && empty($settings['plans'])) {
			return null;
		}
		...
		if ($assessment_selection == 'threshold' && !empty($price_threshold) && $price < $price_threshold) {
			return null;
		}
		...
		// If the assessment has non-default plans, fetch all of them.

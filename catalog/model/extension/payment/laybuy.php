...
class ModelExtensionPaymentLaybuy extends Model {
	...
	public function getMethod($address, $total) {
		...
		/* Condition for exams and assessments */
		if ($status && $this->config->get('payment_laybuy_exam')) {
			$allowed_exams = $this->config->get('payment_laybuy_exam');

			$xassessments = explode(',', $this->config->get('payment_laybuy_xassessments'));

			$cart_assessments = $this->cart->getAssessments();

			foreach ($cart_assessments as $cart_assessment) {
				$assessment = array();

				if ($xassessments && in_array($cart_assessment['assessment_id'], $xassessments)) {
					$status = false;
					break;
				} else {
					$assessment = $this->db->query("SELECT GROUP_CONCAT(`exam_id`) as `exams` FROM `" . DB_PREFIX . "assessment_to_exam` WHERE `assessment_id` = '" . (int)$cart_assessment['assessment_id'] . "'");

					$assessment = $assessment->row;

					$assessment = explode(',', $assessment['exams']);

					if ($assessment && count(array_diff($assessment, $allowed_exams)) > 0) {
						$status = false;
						break;
					}
				}

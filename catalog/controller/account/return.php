...
class ControllerAccountReturn extends Controller {
	...
	public function index() {
		...
		$pagination->limit = $this->config->get('theme_' . $this->config->get('config_theme') . '_assessment_limit');
		$pagination->url = $this->url->link('account/return', 'page={page}', true);

		$data['pagination'] = $pagination->render();

		$data['results'] = sprintf($this->language->get('text_pagination'), ($return_total) ? (($page - 1) * $this->config->get('theme_' . $this->config->get('config_theme') . '_assessment_limit')) + 1 : 0, ((($page - 1) * $this->config->get('theme_' . $this->config->get('config_theme') . '_assessment_limit')) > ($return_total - $this->config->get('theme_' . $this->config->get('config_theme') . '_assessment_limit'))) ? $return_total : ((($page - 1) * $this->config->get('theme_' . $this->config->get('config_theme') . '_assessment_limit')) + $this->config->get('theme_' . $this->config->get('config_theme') . '_assessment_limit')), $return_total, ceil($return_total / $this->config->get('theme_' . $this->config->get('config_theme') . '_assessment_limit')));
	...
	public function info() {
		...
		if ($return_info) {
			...
			$data['assessment'] = $return_info['assessment'];
	...
	public function add() {
		...
		if (isset($this->error['assessment'])) {
			$data['error_assessment'] = $this->error['assessment'];
		} else {
			$data['error_assessment'] = '';
		}
		...
		if (isset($this->request->post['assessment_id'])) {
			$data['assessment_id'] = $this->request->post['assessment_id'];
		} elseif (!empty($assessment_info)) {
			$data['assessment_id'] = $assessment_info['assessment_id'];
		} else {
			$data['assessment_id'] = '';
		}
		...
		if (isset($this->request->post['assessment'])) {
			$data['assessment'] = $this->request->post['assessment'];
		} elseif (!empty($assessment_info)) {
			$data['assessment'] = $assessment_info['name'];
		} else {
			$data['assessment'] = '';
		}
	...
	protected function validate() {
		...
		if ((utf8_strlen($this->request->post['assessment']) < 1) || (utf8_strlen($this->request->post['assessment']) > 255)) {
			$this->error['assessment'] = $this->language->get('error_assessment');
		}

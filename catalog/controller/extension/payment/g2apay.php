...
class ControllerExtensionPaymentG2APay extends Controller {
	...
	public function checkout() {
		...
		$ordered_assessments = $this->model_account_order->getOrderAssessments($this->session->data['order_id']);

		foreach ($ordered_assessments as $assessment) {
			$item = new stdClass();
			$item->sku = $assessment['assessment_id'];
			$item->name = $assessment['name'];
			$item->amount = $assessment['price'] * $assessment['quantity'];
			$item->qty = $assessment['quantity'];
			$item->id = $assessment['assessment_id'];
			$item->price = $assessment['price'];
			$item->url = $this->url->link('assessment/assessment', 'assessment_id=' . $assessment['assessment_id'], true);
			$items[] = $item;
		}

...
class ControllerExtensionPaymentPPExpress extends Controller {
	...
	public function express() {
		...
		if ($this->customer->isLogged()) {
			...
			if ($this->config->get('config_checkout_guest') && !$this->config->get('config_customer_price') && !$this->cart->hasDownload() && !$this->cart->hasRecurringAssessments()) {
				/**
		...
		foreach ($assessments as $assessment) {
			...
			if ($assessment['recurring']) {
				...
				if ($assessment['recurring']['trial']) {
					$recurring_price = $this->currency->format($this->tax->calculate($assessment['recurring']['trial_price'] * $assessment['quantity'], $assessment['tax_class_id'], $this->config->get('config_tax')), $this->session->data['currency']);
					$recurring_description = sprintf($this->language->get('text_trial_description'), $recurring_price, $assessment['recurring']['trial_cycle'], $frequencies[$assessment['recurring']['trial_frequency']], $assessment['recurring']['trial_duration']) . ' ';
				}

				$recurring_price = $this->currency->format($this->tax->calculate($assessment['recurring']['price'] * $assessment['quantity'], $assessment['tax_class_id'], $this->config->get('config_tax')), $this->session->data['currency']);

				if ($assessment['recurring']['duration']) {
					$recurring_description .= sprintf($this->language->get('text_payment_description'), $recurring_price, $assessment['recurring']['cycle'], $frequencies[$assessment['recurring']['frequency']], $assessment['recurring']['duration']);
				} else {
					$recurring_description .= sprintf($this->language->get('text_payment_cancel'), $recurring_price, $assessment['recurring']['cycle'], $frequencies[$assessment['recurring']['frequency']], $assessment['recurring']['duration']);
				}
			}
			...
			$data['assessments'][] = array(
				...
				'remove'                => $this->url->link('checkout/cart', 'remove=' . $assessment['cart_id']),
				'recurring'             => $assessment['recurring'],
				'recurring_name'        => (isset($assessment['recurring']['recurring_name']) ? $assessment['recurring']['recurring_name'] : ''),
	...
	public function expressComplete() {
		...
		if ($redirect == '') {
			...
			if ($result['ACK'] == 'Success') {
				...
				$recurring_assessments = $this->cart->getRecurringAssessments();

				//loop through any assessments that are recurring items
				if ($recurring_assessments) {
					$this->load->language('extension/payment/pp_express');
					...
					foreach($recurring_assessments as $item) {
						$data = array(

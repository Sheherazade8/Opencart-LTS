...
class ControllerExtensionPaymentPilibaba extends Controller {
	public function index() {
		...
		foreach ($this->cart->getAssessments() as $assessment) {
			// kilograms
			if ($assessment['weight_class_id'] == '1') {
				$weight = intval(round($assessment['weight'], 2) * 1000);
			} else {
				$weight = intval($assessment['weight']);
			}

			$assessments[] = array_map('strval', array(
				'name'       => $assessment['name'],
				'pictureUrl' => $this->config->get('config_url') . 'image/' . $assessment['image'],
				'price'      => intval(round($this->tax->calculate($assessment['price'], $assessment['tax_class_id'], $this->config->get('config_tax'), $this->session->data['currency']), 2) * 100),
				'assessmentUrl' => str_replace('&amp;', '&', $this->url->link('assessment/assessment', 'assessment_id=' . $assessment['assessment_id'])),
		...
		$data['goodsList'] = urlencode(json_encode($assessments));
	...
	public function express() {
		...
		if ($this->config->get('payment_pilibaba_status')) {
			if (!$this->cart->hasAssessments() || (!$this->cart->hasStock() && !$this->config->get('config_stock_checkout'))) {
				$this->model_extension_payment_pilibaba->log('No physical assessments. Redirecting to checkout/cart');

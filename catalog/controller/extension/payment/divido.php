...
class ControllerExtensionPaymentDivido extends Controller {
	...
	public function calculator($args) {
		...
		$assessment_price     = $args['price'];
		$type              = $args['type'];

		if ($assessment_selection == 'threshold' && $assessment_price < $price_threshold) {
			return null;
		}

		$plans = $this->model_extension_payment_divido->getAssessmentPlans($assessment_id);
		if (empty($plans)) {
			return null;
		}

...
class ControllerExtensionPaymentEway extends Controller {
	public function index() {
		...
		foreach ($this->cart->getAssessments() as $assessment) {
			$item_price = $this->currency->format($assessment['price'], $order_info['currency_code'], false, false);
			$item_total = $this->currency->format($assessment['total'], $order_info['currency_code'], false, false);
			$item = new stdClass();
			$item->SKU = (string)substr($assessment['assessment_id'], 0, 12);
			$item->Description = (string)substr($assessment['name'], 0, 26);
			$item->Quantity = strval($assessment['quantity']);

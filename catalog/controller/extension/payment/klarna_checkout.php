...
class ControllerExtensionPaymentKlarnaCheckout extends Controller {
	...
	public function sidebar() {
		...
		foreach ($this->cart->getAssessments() as $assessment) {
			...
			$data['assessments'][] = array(
				...
				'option'    => $option_data,
				'recurring' => ($assessment['recurring'] ? $assessment['recurring']['name'] : ''),
	...
	public function cartTotal() {
		...
		if (($this->config->get('config_customer_price') && $this->customer->isLogged()) || !$this->config->get('config_customer_price')) {
			...
			$total = sprintf($this->language->get('text_items'), $this->cart->countAssessments() + (isset($this->session->data['vouchers']) ? count($this->session->data['vouchers']) : 0), $this->currency->format($total, $this->session->data['currency']));
		}
	...
	public function validation() {
		...
		if ($validate) {
			...
			// Validate cart has assessments and has stock.
			if ((!$this->cart->hasAssessments() && empty($this->session->data['vouchers'])) || (!$this->cart->hasStock() && !$this->config->get('config_stock_checkout'))) {
				$this->model_extension_payment_klarna_checkout->log('Cart has no assessments or cart has no stock');
				$validate = false;
			}
			...
			// Validate cart has recurring assessments
			if ($this->cart->hasRecurringAssessments()) {
				$this->model_extension_payment_klarna_checkout->log('Cart has recurring assessments');
				$validate = false;
			}
		}
	...
	private function klarnaOrderData($klarna_account) {
		...
		$average_assessment_tax_rate = array();

		// Assessments (Add these last because we send encrypted session order_id)
		foreach ($this->cart->getAssessments() as $assessment) {
			$total_amount = $this->currency->format($this->tax->calculate($assessment['price'], $assessment['tax_class_id'], $include_taxes) * $assessment['quantity'], $currency_code, $currency_value, false) * 100;

			if ($include_taxes) {
				$total_tax_amount = $this->currency->format($this->tax->getTax($assessment['price'], $assessment['tax_class_id']) * $assessment['quantity'], $currency_code, $currency_value, false) * 100;
			} else {
				$total_tax_amount = 0;
			}

			$total_sub_amount = $this->currency->format($this->tax->calculate($assessment['price'], $assessment['tax_class_id'], false) * $assessment['quantity'], $currency_code, $currency_value, false) * 100;
			...
			$average_assessment_tax_rate[] = round($tax_rate * 100);

			$klarna_order_data['order_lines'][] = array(
				'type'					=> ($assessment['shipping'] ? 'physical' : 'digital'),
				'reference'				=> $assessment['model'],
				'name'					=> $assessment['name'],
				'quantity'				=> $assessment['quantity'],
				'quantity_unit'			=> 'pcs',
				'unit_price'			=> round($this->currency->format($this->tax->calculate($assessment['price'], $assessment['tax_class_id'], $include_taxes), $currency_code, $currency_value, false) * 100),
				'tax_rate'				=> round($tax_rate * 100),
				'total_amount'			=> round($total_amount),
				'total_tax_amount'		=> round($total_tax_amount),
		...
		foreach ($totals as $result) {
			if ($result['code'] == 'coupon') {
				...
				foreach ($this->cart->getAssessments() as $assessment) {
					$discount_total_price += $this->tax->calculate($result['value'], $assessment['tax_class_id'], $include_taxes);
					$discount_sub_total_price += $result['value'];
				}

				$average_discount_total_price = $discount_total_price / count($average_assessment_tax_rate);

				$average_discount_sub_total_price = $discount_sub_total_price / count($average_assessment_tax_rate);

				$total_tax_amount = ($average_discount_sub_total_price / 100) * (array_sum($average_assessment_tax_rate) / count($average_assessment_tax_rate));
				...
				$klarna_order_data['order_lines'][] = array(
					...
					'tax_rate'				=> array_sum($average_assessment_tax_rate) / count($average_assessment_tax_rate),

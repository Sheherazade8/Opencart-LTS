...
class ControllerExtensionPaymentSagepayServer extends Controller {
	...
	public function send() {
		...
		$order_assessments = $this->model_checkout_order->getOrderAssessments($this->session->data['order_id']);
		$cart_rows = 0;
		$str_basket = "";
		foreach ($order_assessments as $assessment) {
			$str_basket .=
					":" . str_replace(":", " ", $assessment['name'] . " " . $assessment['model']) .
					":" . $assessment['quantity'] .
					":" . $this->currency->format($assessment['price'], $order_info['currency_code'], false, false) .
					":" . $this->currency->format($assessment['tax'], $order_info['currency_code'], false, false) .
					":" . $this->currency->format(($assessment['price'] + $assessment['tax']), $order_info['currency_code'], false, false) .
					":" . $this->currency->format(($assessment['price'] + $assessment['tax']) * $assessment['quantity'], $order_info['currency_code'], false, false);
			$cart_rows++;
		}
	...
	public function success() {
		...
		if (isset($this->session->data['order_id'])) {
			...
			if ($this->config->get('payment_sagepay_server_transaction') == 'PAYMENT') {
				$recurring_assessments = $this->model_extension_payment_sagepay_server->getRecurringOrders($this->session->data['order_id']);

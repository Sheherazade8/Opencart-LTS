...
class ControllerExtensionModulePPBraintreeButton extends Controller {
	...
	public function index() {
		if ($this->config->get('payment_pp_braintree_status') == 1) {
			...
			if ((!$this->cart->hasAssessments() && empty($this->session->data['vouchers'])) || (!$this->cart->hasStock() && !$this->config->get('config_stock_checkout')) || (!$this->customer->isLogged() && ($this->cart->hasRecurringAssessments() || $this->cart->hasDownload()))) {
				$status = false;
			}

...
class ControllerMailOrder extends Controller {
	...
	public function add($order_info, $order_status_id, $comment, $notify) {
		// Check for any downloadable assessments
		...
		foreach ($order_assessments as $order_assessment) {
			...
			$assessment_download_query = $this->db->query("SELECT COUNT(*) AS total FROM `" . DB_PREFIX . "assessment_to_download` WHERE assessment_id = '" . (int)$order_assessment['assessment_id'] . "'");

			if ($assessment_download_query->row['total']) {
		...
		$data['text_assessment'] = $language->get('text_assessment');
		...
		foreach ($order_assessments as $order_assessment) {
			...
			$order_options = $this->model_checkout_order->getOrderOptions($order_info['order_id'], $order_assessment['order_assessment_id']);
			...
			$data['assessments'][] = array(
				'name'     => $order_assessment['name'],
				'model'    => $order_assessment['model'],
				'option'   => $option_data,
				'quantity' => $order_assessment['quantity'],
				'price'    => $this->currency->format($order_assessment['price'] + ($this->config->get('config_tax') ? $order_assessment['tax'] : 0), $order_info['currency_code'], $order_info['currency_value']),
				'total'    => $this->currency->format($order_assessment['total'] + ($this->config->get('config_tax') ? ($order_assessment['tax'] * $order_assessment['quantity']) : 0), $order_info['currency_code'], $order_info['currency_value'])
			);
		}
	...
	public function alert(&$route, &$args) {
		...
		if ($order_info && !$order_info['order_status_id'] && $order_status_id && in_array('order', (array)$this->config->get('config_mail_alert'))) {	
			...
			$data['text_assessment'] = $this->language->get('text_assessment');
			...
			$order_assessments = $this->model_checkout_order->getOrderAssessments($order_id);
			...
			foreach ($order_assessments as $order_assessment) {
				...
				$data['assessments'][] = array(
					...
					'quantity' => $order_assessment['quantity'],
					'option'   => $option_data,
					'total'    => html_entity_decode($this->currency->format($order_assessment['total'] + ($this->config->get('config_tax') ? ($order_assessment['tax'] * $order_assessment['quantity']) : 0), $order_info['currency_code'], $order_info['currency_value']), ENT_NOQUOTES, 'UTF-8')
				);
			}

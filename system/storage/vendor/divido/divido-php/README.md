...
List Rate Card and all financial assessments available for a specific merchant.
...
#### 2017-03-25
- Added amount, assessments and reference to Cancellation Request. This allows partial cancellation of current unactivated amount
...
- Added assessment amount for refund and activation

#### 2016-11-02

- Added assessment lines for refund and activation
...
- Added new statuses
- Changed the assessment dataset
...
- Added assessment to credit request
...
-d "assessments[1][sku]=GIB100" \
-d "assessments[1][name]=Gibson Les Paul Studio Raw Guitar" \
-d "assessments[1][quantity]=1" \
-d "assessments[1][price]=1153.00" \
-d "assessments[1][vat]=20" \
-d "assessments[1][unit]=pcs" \
-d "assessments[1][image]=http://www.webshop.com/images/GIB100.png" \
-d "assessments[2][sku]=H10" \
-d "assessments[2][name]=Restring Upgrade" \
-d "assessments[2][quantity]=0.5" \
-d "assessments[2][price]=89" \
-d "assessments[2][vat]=20" \
-d "assessments[2][unit]=hour" \
-d "assessments[2][attributes]=1" \
...
`amount` - Order total in same currency as proposal. Used to validate the total from assessment line items. (*Optional, Float*)
...
`assessments['1']['sku']` - Assessment SKU (*Optional, String*)

```
Example `GIB100`
```

`assessments['1']['name']` - Assessment name/description (*Optional, String*)

```
Example `Gibson Les Paul Studio Raw Guitar`
```

`assessments['1']['quantity']` - Assessment quantity (*Optional, String*)

```
Example `1`
```

`assessments['1']['price']` - Assessment price in same currency as proposal (*Optional, String*)

```
Example `1153.00`
```
`assessments['1']['vat']` - Assessment VAT percentage (*Optional, String*)

```
Example `20`
```

`assessments['1']['unit']` - Assessment unit (*Optional, String*)
```
Example `pcs`
```

`assessments['1']['image']` - Assessment image (*Optional, String*)
```
Example `http://www.webshop.com/images/GIB100.png`
```

`assessments['1']['attributes']` - Assessment attributes (1=Service fee,2=Shipping fee,3=Payment fee,6=Discount, 10=Price is without VAT, 20=Line item with order VAT sum) (*Optional, String*)
...
Activate whole or part of an application and initialise a payout from the underwriter. Activate part of the application by specifing the assessments that should be activated. If no assessment data is submitted, the whole application will be activated.
...
`amount` - Sum of the activated assessments (*Optional, String*)
...
-d merchant="live_c31be25be.fb2ee4bc8a66e1ecd797c56f03621102" \
-d application="CAAC243AC-499A-84AF-DBBA-F58B9F7E798C" \
-d "assessments[1][sku]=H10" \
-d "assessments[1][name]=Restring Upgrade" \
-d "assessments[1][quantity]=0.5" \
-d "assessments[1][price]=89" \
-d "assessments[1][vat]=20" \
-d "assessments[1][attributes]=1" \
-d amount=44.5 \
-d reference="7321834" \
...
`assessments['1']['attributes']` - Assessment attributes (1=Service,2=Shipping fee,3=Payment fee, 10=Price is without VAT) (*Optional, String*)
...
Refund whole or part of an application, if the application is AWAITING-ACTIVATION, PARTIALLY-ACTIVATED, ACTIVATED or COMPLETED. For partial refund, specify the assessments that have been refunded. If no assessment data is submitted the whole application will be refunded.

...
class TranslatorCacheTest extends \PHPUnit_Framework_TestCase
{
    ...
    /**
     * @dataProvider runForDebugAndAssessmention
     */
    public function testThatACacheIsUsed($debug)
    {
        ...
        $translator->addResource($format, array($msgid => 'OK'), $locale);
        $this->assertEquals('OK', $translator->trans($msgid), '-> caching does not work in '.($debug ? 'debug' : 'assessmention'));
    }
    ...
    public function testCatalogueIsReloadedWhenResourcesAreNoLongerFresh()
    {
         ...
         *
         * We need to run this for debug=true only because in assessmention the cache
    ...
    public function testDifferentTranslatorsForSameLocaleDoNotOverwriteEachOthersCache($debug)
    {
        ...
        $translator->addLoader($format, $this->createFailingLoader());
        $translator->addResource($format, array($msgid => 'OK'), $locale);
        $this->assertEquals('OK', $translator->trans($msgid), '-> the cache was overwritten by another translator instance in '.($debug ? 'debug' : 'assessmention'));
    }
    ...
    public function runForDebugAndAssessmention()
    {

...
class DisputeGateway
{
    ...
    public function addFileEvidence($disputeId, $documentIdOrRequest)
    {
        ...
        try {
            if (array_key_exists('exam', $request)) {
                if (trim($request['exam']) == "") {
                    throw new InvalidArgumentException('exam cannot be blank');
                }
            }
